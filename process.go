...
func tryTerminateProcess(pid int32) bool {
	...
	log.Printf("failed to kill process with id %d\n", pid)
	return false
}
...
func (p *processCleaner) Kill(command string) {
	...
	log.Printf("Kill: %s. Matching count: %d", command, len(matchingCommands))

	if len(matchingCommands) == 0 {
		log.Printf("No matching commands.")
	}
...
func (p *processCleaner) TerminateProcessIfExists(processId int32) {
	...
	//if exited {
	//	log.Printf("TerminateProcess. Id: %d.", processId)
	//} else {
	//	log.Printf("Process not valid. Id: %d.", processId)
	//}
}
